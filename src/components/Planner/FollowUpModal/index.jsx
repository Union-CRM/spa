...
const FollowUpModal = () => {
    ...
    const clientOption= client?client.filter((c) => c.status === "Active").map((c) => ({value:c.id, label:c.client})) :[]
    
    useEffect(() => {
        setGuests(plannerEdit.guest?plannerEdit.guest.map((g)=>({value:g.client_id,label:g.client_name})):[])
    },[])
   ...
   const handleSubmit=(e)=>{
        ...
        const newPlanner = {
            ...
            date: date.toISOString().substring(0, 10) +" "+date.toLocaleTimeString(),
  ...
  return (
    <Container>
        ...
        <Form onSubmit={handleSubmit}>
            ...
            <InputSubject type="text" value={plannerEdit.subject_title} disabled></InputSubject>
            <PositionLabel>Client Name</PositionLabel>
            <InputSubject type="text" value={plannerEdit.client_name} required disabled></InputSubject>
            <PositionLabel>Email</PositionLabel>
            <InputSubject type="text" value={plannerEdit.client_email} disabled></InputSubject>
            <PositionLabel>Business</PositionLabel>
            <InputSubject type="text" value={plannerEdit.business} disabled></InputSubject>
            <PositionLabel>Release Train</PositionLabel>
            <InputSubject type="text" value={plannerEdit.release_title} disabled></InputSubject>
            <PositionTag>
                <GuestComponent options={clientOption} tags={guests} set={(g) => setGuests(g)} label={"Guests"} sizeHeight={"3.5vh"} width={"9vw"} widths={"15vw"} heights={"12vh"} marginLeft={"40%"}/>
            </PositionTag>
            <PositionSingle>
                <SingleSelect  
                    placeholder={""}
